// SingleCycleLinkedList.cpp
#include "Header.hpp"

// 构造函数()
LinkList::LinkList(){
    m_head = new Node;  // ......头指针动态分配内存指向头节点
    m_head->next = m_head;  // ......头节点指针域指向自己形成循环
}
...
bool LinkList::IsEmpty(){
    if(m_head->next == m_head){  // ......若头节点指针域(后继节点)为头节点
        return true;  // .....链表为空
    }
...
void LinkList::Prepend(int data){
    // 待插入节点
    Node* pNode = new Node;  // ......创建指针并动态分配内存指向新节点
...
void LinkList::Append(int data){
    ...
    while(p->next != m_head){  // ......若节点指针域(后继节点)不为头节点
        p = p->next;  // ......指针指向下一个节点
    }

    // 节点插入
    pNode->next = p->next;  // ......新节点指针域指向头节点
...
bool LinkList::Insert(int pos, int data){
    ...
    // 找到插入位置前驱节点
    Node* p = m_head;  // ......创建指针指向头节点
    for(int i = 0; i < pos - 1; i++){  // ......循环直到前驱节点位置
        p = p->next;  // ......指针指向下一个节点
    }
...
bool LinkList::Remove(int pos){
    ...
    // 节点删除
    p->next = delNode->next;  // ......前驱节点指针域指向后继节点
